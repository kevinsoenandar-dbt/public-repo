...
{% if execute %}
  ...
  {% for schema, roles in schema_grants.items() %}
    ...
    {% set roles_to_check = results | list | selectattr('privilege', 'equalto', 'USAGE') | list %}
    {% set roles_to_check = roles_to_check | map(attribute='grantee_name') | list %}
    ...
    {% for revoke_role in roles_to_check %}
        {% if revoke_role not in roles %}
            ...
            {% do run_query("REVOKE USAGE ON SCHEMA " ~ schema ~ " FROM " ~ revoke_role) %}
        {% endif %}
    {% endfor %}
    ...
    {% for grant_role in roles %}
        {% if grant_role not in roles_to_check %}
          ...
          {% do run_query("GRANT USAGE ON SCHEMA " ~ schema ~ " TO " ~ grant_role) %}
        {% endif %}
    {% endfor %}
